//! Gemma/CodeGemma forward pass with per-layer activation capture
//!
//! Custom implementation that runs layer-by-layer to capture
//! intermediate activations for PLIP probing experiments.
//!
//! Based on the Gemma architecture from Google (CodeGemma 7B-IT).
...
/// Model configuration (matches HuggingFace config.json for Gemma/CodeGemma)
...
/// Gemma-style RMSNorm (adds 1.0 to weight)
struct GemmaRmsNorm {
    weight: Tensor,
    eps: f64,
}
...
impl Attention {
    fn load(vb: VarBuilder, config: &GemmaConfig) -> Result<Self> {
        let head_dim = config
            .head_dim
    ...
    fn forward_with_cache(
        ...
        *cache_v = Some(v.clone());
        ...
        // Ensure tensors are contiguous for matmul (Gemma-specific)
        let q = q.contiguous()?;
        let k = k.contiguous()?;
        let v = v.contiguous()?;
        ...
        // Create generation mask for the new positions attending to all positions
        let mask =
...
/// MLP block (Gemma style - GeGLU with gate/up/down projections)
...
impl MLP {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let gate = gate.gelu()?; // Gemma uses GELU, not SiLU
...
/// Custom Gemma model with per-layer activation capture
pub struct PlipGemma {
...
impl PlipGemma {
    ...
    pub fn load(model_id: &str, device: &Device, dtype: DType) -> Result<Self> {
        info!("Loading Gemma from: {}", model_id);
        ...
        let config: GemmaConfig = serde_json::from_str(&config_str)?;
    ...
    pub fn forward_with_cache(&self, input_ids: &Tensor) -> Result<(Tensor, ActivationCache)> {
        ...
        // Embedding - Gemma normalizes embeddings by sqrt(hidden_size)
    ...
    pub fn forward_with_kv_cache(
        ...
        // Embedding with Gemma's sqrt(hidden_size) normalization
        let mut hidden = self.embed_tokens.forward(input_ids)?;
        let normalizer = (self.hidden_size as f64).sqrt();
        hidden = (hidden * normalizer)?;
...
impl PlipBackend for PlipGemma {
