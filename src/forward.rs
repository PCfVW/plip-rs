//! StarCoder2 forward pass with per-layer activation capture
//!
...
impl Attention {
    fn load(vb: VarBuilder, config: &StarCoder2Config) -> Result<Self> {
...
impl MLP {
    fn load(vb: VarBuilder, config: &StarCoder2Config) -> Result<Self> {
        let c_fc = linear_no_bias(config.hidden_size, config.intermediate_size, vb.pp("c_fc"))?;
        let c_proj = linear_no_bias(
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        // StarCoder2 uses GELU activation
...
/// Custom StarCoder2 model with per-layer activation capture
pub struct PlipStarCoder2 {
...
impl PlipStarCoder2 {
    ...
    pub fn load(model_id: &str, device: &Device, dtype: DType) -> Result<Self> {
        info!("Loading StarCoder2 from: {}", model_id);
    ...
    pub fn forward_with_kv_cache(
        ...
        // Embedding (no special normalization for StarCoder2)
        let mut hidden = self.embed_tokens.forward(input_ids)?;
