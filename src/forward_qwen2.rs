...
pub struct PlipQwen2 {
    ...
    norm: RmsNorm,
    /// Language modeling head (may be separate or tied to embeddings)
    ...
    hidden_size: usize,
    vocab_size: usize,
    /// Whether embeddings are tied to lm_head (stored for reference)
    #[allow(dead_code)]
...
impl PlipQwen2 {
    ...
    pub fn load(model_id: &str, device: &Device, dtype: DType) -> Result<Self> {
        ...
        // Load lm_head if not using weight tying
    ...
    pub fn project_to_vocab(&self, hidden: &Tensor) -> Result<Tensor> {
        let logits = if let Some(ref lm_head) = self.lm_head {
            ...
            hidden.matmul(&self.embed_weight.t()?)?
        };
        Ok(logits)
    }
    ...
    pub fn forward_with_kv_cache(
        ...
        let logits = if let Some(ref lm_head) = self.lm_head {
            ...
            // Use tied embeddings
            last_hidden.matmul(&self.embed_weight.t()?)?
        };
