...
//!
//! To run with the 2.5M CLT (requires ~190 GB disk and a pre-generated reference):
//!   cargo run --release --example reproduce_pipeline -- \
//!       --clt-repo mntss/clt-gemma-2-2b-2.5M \
//!       --clt-ref-file scripts/clt_reference_2.5M.json \
...
struct Args {
    ...
    /// Reference JSON for CLT validation (generated by scripts/clt_reference.py).
    /// Change this when switching to a different CLT (e.g. `scripts/clt_reference_2.5M.json`).
    #[arg(long, default_value = "scripts/clt_reference_426k.json")]
    clt_ref_file: String,
...
fn main() {
    ...
    eprintln!(
        "║  Model:  {}{}║",
        &args.model,
        " ".repeat(42_usize.saturating_sub(args.model.len()))
    );
    eprintln!(
        "║  Output: {}{}║",
        out.display(),
        " ".repeat(42_usize.saturating_sub(out.display().to_string().len()))
    );
    eprintln!(
        "║  Steps:  {}{}║",
        &range_str,
        " ".repeat(42_usize.saturating_sub(range_str.len()))
    );
    eprintln!("╚════════════════════════════════════════════════════════════╝");
    ...
    if args.skip_clt_validation {
        ...
        run_step(
            2,
            "Validate CLT encoding against Python reference",
            "CLT validation",
            start,
            end,
            || run_example("validate_clt", &["--ref-file", &args.clt_ref_file]),
        );
    }
    ...
    run_step(
        3,
        "CLT logit-shift acceptance test (GPU)",
        "CLT logit-shift",
        start,
        end,
        || {
            run_example(
                "clt_logit_shift",
                &["--model", &args.model, "--clt-repo", &args.clt_repo],
            )
        },
    );
    ...
    run_step(
        4,
        "Phase 1: Detect planning signal via attention (GPU)",
        "Attention layer scan",
        start,
        end,
        || {
            run_example(
    ...
    run_step(
        5,
        "Phase 2a: CLT steering experiments, Methods 3-6 (GPU)",
        "CLT steering (5 runs)",
        start,
        end,
        || {
    ...
    run_step(
        6,
        "Phase 2b: Attention steering comparison (GPU)",
        "Attention steering",
        start,
        end,
        || {
    ...
    run_step(
        7,
        "Phase 2c: Cross-mechanism evaluation",
        "Cross-mechanism evaluation",
        start,
        end,
        || {
            run_example(
    ...
    run_step(
        8,
        "Bottom-up: Scan CLT features against vocabulary (GPU)",
    ...
    run_step(
        9,
        "Find rhyme pairs via CMU Pronouncing Dictionary",
        "Rhyme pair discovery",
        start,
        end,
        || {
            run_example(
    ...
    run_step(
        10,
        "Planning detection V2: completion-style probes (GPU)",
        "Planning detection",
        start,
        end,
        || {
    ...
    run_step(
        11,
        "Version C: Multi-layer causal position sweep (GPU)",
        "Steering sweep (Version C)",
        start,
        end,
        || {
            run_example(
    ...
    run_step(
        12,
        "Version D: Suppress + inject sweep (GPU)",
        "Suppress + inject (Version D)",
        start,
        end,
        || {
            run_example(
    ...
    run_step(
        13,
        "Analyze suppress + inject results",
        "Analysis",
        start,
        end,
        || run_example("analyze_suppress_inject", &["--input", &si_str]),
    );
    ...
    eprintln!("║                                                          ║");
    eprintln!(
        "║  Steps:  {range_str:>7} ({start}-{end} of {TOTAL_STEPS})                            ║"
    );
    eprintln!(
        "║  Total time: {total_mins:>3}m {total_rem:>2}s                                   ║"
    );
    eprintln!(
        "║  Output dir: {}{}║",
        out.display(),
