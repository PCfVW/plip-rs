...
//!
//! This experiment tests the causal hypothesis that attention from test markers
//! to function tokens is critical for test preservation behavior.
//!
//! ## Hypothesis
//!
//! If test marker → function token attention is causally important for
//! "copying" expected values, then knocking out these edges should:
...
//!    - Run knockout forward pass (zero attention from marker to function tokens)
//!    - Compute KL divergence
//! 3. Compare Python vs Rust with statistical testing (Welch's t-test)
//!
//! Usage:
//!   cargo run --release --example ablation_experiment
//!   cargo run --release --example ablation_experiment -- --model "Qwen/Qwen2.5-Coder-7B-Instruct"
//!   cargo run --release --example ablation_experiment -- --layer 16 --all-heads
//!   cargo run --release --example ablation_experiment -- --output results.json
...
#[derive(Parser)]
#[command(name = "ablation_experiment")]
#[command(about = "Test whether attention knockout breaks test preservation")]
struct Args {
    ...
    /// Knockout ALL outgoing attention from marker (not just to function tokens)
    #[arg(long)]
    all_edges: bool,

    /// Target all heads (default: yes)
...
fn main() -> Result<()> {
    ...
    println!("=== Ablation Experiment: Does Removing Attention Break Preservation? ===\n");
    ...
    let target_layer = args.layer.unwrap_or_else(|| {
        // Heuristic: use ~60% through the model (where semantic processing happens)
        let n_layers = model.n_layers();
        if n_layers >= 32 {
            20 // For 32+ layer models
        } else if n_layers >= 28 {
            16 // For 28-layer models (Qwen2.5-Coder-7B)
        } else {
            14 // For smaller models (Qwen2.5-Coder-3B)
        }
    });
...
/// Run the main ablation experiment
fn run_experiment(
    model: &PlipModel,
    corpus: &Corpus,
    layer: usize,
    ...
    println!("Running ablation at layer {}\n", layer);
...
/// Process a single sample with a single layer
fn process_sample(
    model: &PlipModel,
    sample: &Sample,
    language: &str,
    ...
    process_sample_layers(model, sample, language, is_baseline, layer, layer, _args)
}
...
fn process_sample_layers(
    ...
    if !args.all_edges && target_token_positions.is_empty() {
        anyhow::bail!("No target tokens found");
    }

    // Build knockout spec
    let spec = if args.all_edges {
        // Knockout ALL incoming attention TO marker position from subsequent tokens
        // This tests: "what if later tokens couldn't see the marker at all?"
        let seq_len = encoding.len();
        if args.verbose {
            eprintln!("  [DEBUG] All-edges mode: knocking out attention TO marker pos {} from all {} tokens",
                     marker_token_pos, seq_len);
        }
        if layer_start == layer_end {
            KnockoutSpec::new()
                .layer(layer_start)
                .to_position(marker_token_pos)
        } else {
            KnockoutSpec::new()
        ...
        // Knockout only marker -> function token edges
        if layer_start == layer_end {
            KnockoutSpec::new()
                .layer(layer_start)
                .from_to_positions(marker_token_pos, &target_token_positions)
        } else {
            KnockoutSpec::new()
                .layer_range(layer_start, layer_end)
                .from_to_positions(marker_token_pos, &target_token_positions)
    ...
    // Run ablation
    let result = model.forward_with_intervention(&sample.code, &spec)?;
    ...
    // For all_edges mode, we knockout attention to all tokens
    let edges_knocked_out = if args.all_edges {
        encoding.len() // All outgoing edges from marker
    } else {
        target_token_positions.len()
    };
    ...
    Ok(SampleResult {
        ...
        kl_divergence: kl,
        marker_token_pos,
        target_token_positions: target_token_positions.clone(),
        edges_knocked_out,
        top_changed_tokens,
    })
}
...
fn run_layer_scan(model: &PlipModel, corpus: &Corpus, args: &Args) -> Result<()> {
    println!("=== Layer Scan: Finding Most Important Layers ===\n");
...
fn run_experiment_window(
    ...
    println!(
        "Running ablation on layers {}-{} (window of {} layers)\n",
        layer_start,
        layer_end,
...
fn run_window_scan(
    ...
    println!(
        "=== Window Scan: Testing Increasing Windows Around Layer {} ===\n",
        center
    );
...
fn run_sliding_window_scan(
    ...
    if window_size > n_layers {
        anyhow::bail!(
            "Window size {} exceeds number of layers {}",
            window_size,
            n_layers
        );
    }

    println!("=== Sliding Window Scan: Window Size {} ===\n", window_size);
    println!(
        "Model: {} ({} layers, {} heads)",
        args.model,
        n_layers,
    ...
    for (start, end, py_kl, rust_kl) in &results {
        ...
        println!(
            "{:5} | {:2}-{:<2}      | {:.6}   | {:.6}   | {:8.2}x  | {}",
            start, start, end, py_kl, rust_kl, ratio, peak_marker
        );
    }

    // Summary
    println!("\n=== Summary ===");
    println!("Window size: {} layers", window_size);
    println!(
...
fn print_results_window(results: &WindowExperimentResults) {
    println!("\n============================================================");
    println!("WINDOW EXPERIMENT RESULTS");
    println!("============================================================\n");
    ...
    if py_mean > 0.01 || rust_mean > 0.01 {
        println!("FINDING: Multi-layer knockout has measurable impact on predictions.");
        println!(
            "         Knocking out layers {}-{} significantly changes model behavior.",
            results.layer_start, results.layer_end
        );
    } else {
        println!("FINDING: Multi-layer knockout has minimal impact on predictions.");
...
fn welch_t_test(group1: &[f32], group2: &[f32]) -> (f32, f32) {
    ...
    // For simplicity, use the approximation for large df
...
fn print_results(results: &ExperimentResults) {
    ...
    println!("Model: {}", results.model);
    println!("Layer tested: {}", results.layer);
    println!("Number of heads: {}\n", results.n_heads);
    ...
    if py_mean > 0.01 || rust_mean > 0.01 {
        println!("FINDING: Attention knockout has measurable impact on predictions.");
        println!("         This supports the hypothesis that test marker -> function token");
        println!("         attention is causally important for preservation behavior.");
    } else {
        println!("FINDING: Attention knockout has minimal impact on predictions.");
    ...
    if results.significant_difference {
        if py_mean > rust_mean {
            ...
            println!("\nFINDING: Rust tests are MORE affected by knockout than Python doctests.");
            println!("         This suggests different preservation mechanisms per language.");
        }
    } else {
