...
//!   cargo run --release --example `validate_clt`               # 426K CLT (default)
//!   cargo run --release --example `validate_clt` -- \
//!       --ref-file scripts/clt_reference_2.5M.json             # 2.5M CLT
...
#[derive(Parser)]
#[command(name = "validate_clt")]
#[command(about = "Validate plip-rs CLT encoding against Python reference")]
struct Args {
    /// Reference JSON generated by `scripts/clt_reference.py`.
    /// Pass a different file to validate a different CLT (e.g. the 2.5M CLT).
...
fn main() -> Result<()> {
    ...
    // Load reference data
    let ref_path = &args.ref_file;
    let ref_text = std::fs::read_to_string(ref_path).with_context(|| {
        format!(
            "Failed to read {}. Run: python scripts/clt_reference.py",
            ref_path.display()
        )
    })?;
